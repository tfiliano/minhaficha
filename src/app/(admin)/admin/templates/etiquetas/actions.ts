  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value !== null && typeof value === "object") {
      if (Array.isArray(value)) {
        ...
        if (value !== obj) {
          // Evita auto-referências diretas
    ...
    try {
      ...
      const template = {
        ...
        campos: campos.map((campo) => ({
          ...
          enabled: campo?.hasOwnProperty("enabled")
            ? Boolean(campo.enabled)
            : true,
...
export async function generateZplCode(
      ...
      switch (field.fieldType) {
        ...
        case "static": {
          ...
          if (field.fullWidth && field.reversed) {
            ...
            zpl.push(
              `^GB${fieldWidth},${field.fontSize * 1.5},${
                field.fontSize * 0.1
              },B^FS`
            );
        ...
        case "line": {
          ...
          zpl.push(`^FO${field.x},${field.y}`);
          zpl.push(
            `^GB${field.lineWidth || 100},${field.lineHeight || 1},${
              field.lineHeight || 1
            }^FS`
          );
          break;
        ...
        case "box": {
          ...
          zpl.push(`^FO${field.x},${field.y}`);
          zpl.push(
            `^GB${field.boxWidth || 100},${field.boxHeight || 50},${
              field.boxBorderWidth || 1
            }^FS`
          );
          break;
        }
